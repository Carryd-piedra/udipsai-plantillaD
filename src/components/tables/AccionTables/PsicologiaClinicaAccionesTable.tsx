...
export default function PsicologiaClinicaAccionesTable() {
  ...
  const loadFichas = async () => {
    try {
      setLoading(true);
      const data = await fichasService.listarPsicologiaClinica();
      setFichas(data);
    } catch (error) {
  ...
  const confirmDelete = async () => {
    if (fichaToDelete) {
      try {
        await fichasService.eliminarPsicologiaClinica(fichaToDelete);
  ...
  return (
    <div className="space-y-6">
      <TableActionHeader
        ...
        onNew={() => navigate("/psicologia-clinica/nuevo")}
                  ...
                  >
                    ...
                    <TableCell className="px-5 py-3 text-center text-theme-xs text-gray-700 dark:text-gray-300">
                      <div className="flex justify-center gap-2">
                        {hasPermission("PERM_PSICOLOGIA_CLINICA_EDITAR") && (
                          <button
                            className="text-blue-600 hover:text-blue-800 font-medium text-sm"
                            onClick={() =>
                              navigate(
                                `/psicologia-clinica/editar/${ficha.paciente.id}`
                              )
                            }
                          >
                            Editar
                        ...
                        {hasPermission("PERM_PSICOLOGIA_CLINICA_ELIMINAR") && (
                          <button
