...
export default function PsicologiaEducativaAccionesTable() {
  ...
  const loadFichas = async () => {
    try {
      setLoading(true);
      const data = await fichasService.listarPsicologiaEducativa();
      setFichas(data);
    } catch (error) {
  ...
  const confirmDelete = async () => {
    if (fichaToDelete) {
      try {
        await fichasService.eliminarPsicologiaEducativa(fichaToDelete);
                  ...
                  >
                    ...
                    <TableCell className="px-5 py-3 text-center text-theme-xs text-gray-700 dark:text-gray-300">
                      <div className="flex justify-center gap-2">
                        {hasPermission("PERM_PSICOLOGIA_EDUCATIVA_EDITAR") && (
                          <button
                            className="text-blue-600 hover:text-blue-800 font-medium text-sm"
                            onClick={() =>
                              navigate(
                                `/psicologia-educativa/editar/${ficha.paciente.id}`
                              )
                            }
                          >
                            Editar
                        ...
                        {hasPermission("PERM_PSICOLOGIA_EDUCATIVA_ELIMINAR") && (
                          <button
